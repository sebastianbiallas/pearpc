...
{
	if (invalid != NULL && ((insn & (1 << 21)) == 0 || (insn & (1 << 15)) == 0)) *invalid = 1;
	if ((insn & 0x8000) != 0) {
		return (insn & 0xfffc) - 0x10000;
	} else {
		return insn & 0xfffc;
...
static int valid_bo(uint32 value)
{
...
#define BDM (7)
  { 16, 0, extract_bdm, PPC_OPERAND_RELATIVE | PPC_OPERAND_SIGNED },

  /* The BD field in a B form instruction when the - modifier is used
	and absolute address is used.  */
#define BDMA (8)
  { 16, 0, extract_bdm, PPC_OPERAND_ABSOLUTE | PPC_OPERAND_SIGNED },

  /* The BD field in a B form instruction when the + modifier is used.
	This sets the y bit of the BO field appropriately.  */
#define BDP (9)
  { 16, 0, extract_bdp, PPC_OPERAND_RELATIVE | PPC_OPERAND_SIGNED },

  /* The BD field in a B form instruction when the + modifier is used
	and absolute addressing is used.  */
#define BDPA (10)
  { 16, 0, extract_bdp, PPC_OPERAND_ABSOLUTE | PPC_OPERAND_SIGNED },
...
#define D (19)
  ...
  /* The DS field in a DS form instruction.  This is like D, but the
	lower two bits are forced to zero.  */
#define DS (20)
  { 16, 0, extract_ds, PPC_OPERAND_PARENS | PPC_OPERAND_SIGNED },

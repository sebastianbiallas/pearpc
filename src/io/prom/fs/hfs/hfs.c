...
int validvname(const char *name)
{
  ...
  if (strchr(name, '/'))
    ERROR(EINVAL, "volume name may not contain slashes");
 ...
 */
hfsvol *hfs_mount(const void *devicehandle, int pnum, int mode)
{
  hfsvol *vol, *check;
  ...
  for (check = hfs_mounts; check; check = check->next)
    {
      if (check->pnum == pnum && v_same(check, devicehandle) == 1)
	{
  ...
  switch (mode & HFS_MODE_MASK)
    ...
    default:
      ...
      if (v_open(vol, devicehandle, HFS_MODE_RDONLY) == -1)
	goto fail;
    }
...
/*
 * NAME:	hfs->opendir_by_id()
 * DESCRIPTION:	prepare to read the contents of a directory
 */
hfsdir *hfs_opendir_by_id(hfsvol *vol, int id)
{
  hfsdir *dir = 0;
  CatKeyRec key;
/*  CatDataRec data;*/
 ...
 * DESCRIPTION:	prepare a file for I/O
 */
hfsfile *hfs_open_by_dirent(hfsvol *vol, hfsdirent *dirent)
{
  hfsfile *file = 0;
  int found;
  ...
  memcpy(file->name, dirent->name, 31);
  file->name[31] = 0;
  file->parid = dirent->parid;

  found = v_catsearch(vol, file->parid, file->name, &file->cat, 0, NULL);
  if (found != 1)
    goto fail;
 ...
 */
int hfs_mkpart(const void *devicehandle, unsigned long len)
{
  hfsvol vol;
 ...
 */
int hfs_nparts(const void *devicehandle)
{
  hfsvol vol;
  int nparts, found;
  ApplePartition map;
 ...
 */
int hfs_format(const void *devicehandle, int pnum, int mode, const char *vname,

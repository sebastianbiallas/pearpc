...
typedef signed short	APPLEInteger;
...
typedef struct {
  APPLEInteger	sbSig;		/* device signature (should be 0x4552) */
  APPLEInteger	sbBlkSize;	/* block size of the device (in bytes) */
  LongInt	sbBlkCount;	/* number of blocks on the device */
  APPLEInteger	sbDevType;	/* reserved */
  APPLEInteger	sbDevId;	/* reserved */
  LongInt	sbData;		/* reserved */
  APPLEInteger	sbDrvrCount;	/* number of driver descriptor entries */
  LongInt	ddBlock;	/* first driver's starting block */
  APPLEInteger	ddSize;		/* size of the driver, in 512-byte blocks */
  APPLEInteger	ddType;		/* driver operating system type (MacOS = 1) */
  APPLEInteger	ddPad[243];	/* additional drivers, if any */
} Block0;

typedef struct {
  APPLEInteger	pmSig;		/* partition signature (0x504d or 0x5453) */
  APPLEInteger	pmSigPad;	/* reserved */
...
typedef struct {
  ...
  APPLEInteger	bbVersion;	/* boot blocks version number */
  APPLEInteger	bbPageFlags;	/* used internally */
  ...
  APPLEInteger	bbCntFCBs;	/* number of FCBs to allocate */
  APPLEInteger	bbCntEvts;	/* number of event queue elements */
...
typedef struct {
  APPLEInteger	v;		/* vertical coordinate */
  APPLEInteger	h;		/* horizontal coordinate */
} Point;

typedef struct {
  APPLEInteger	top;		/* top edge of rectangle */
  APPLEInteger	left;		/* left edge */
  APPLEInteger	bottom;		/* bottom edge */
  APPLEInteger	right;		/* right edge */
} Rect;

typedef struct {
  Rect		frRect;		/* folder's rectangle */
  APPLEInteger	frFlags;	/* flags */
  Point		frLocation;	/* folder's location */
  APPLEInteger	frView;		/* folder's view */
} DInfo;

typedef struct {
  Point		frScroll;	/* scroll position */
  LongInt	frOpenChain;	/* directory ID chain of open folders */
  APPLEInteger	frUnused;	/* reserved */
  APPLEInteger	frComment;	/* comment ID */
...
typedef struct {
  ...
  APPLEInteger	fdFlags;	/* flags */
  Point		fdLocation;	/* file's location */
  APPLEInteger	fdFldr;		/* file's window */
} FInfo;

typedef struct {
  APPLEInteger	fdIconID;	/* icon ID */
  APPLEInteger	fdUnused[4];	/* reserved */
  APPLEInteger	fdComment;	/* comment ID */
  LongInt	fdPutAway;	/* home directory ID */
} FXInfo;

typedef struct {
  APPLEInteger	drSigWord;	/* volume signature (0x4244 for HFS) */
  LongInt	drCrDate;	/* date and time of volume creation */
  LongInt	drLsMod;	/* date and time of last modification */
  APPLEInteger	drAtrb;		/* volume attributes */
  ...
  APPLEInteger	drVSeqNum;	/* volume backup sequence number */
...
typedef struct {
  ...
  union {
    struct {  /* cdrDirRec */
      APPLEInteger	dirFlags;	/* directory flags */

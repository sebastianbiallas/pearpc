...
// m must be uint64
#define FPD_PACK_VAR(f, s, e, m)                                                                                       \
    (f) = ((s) ? FPU_SIGN_BIT : 0ULL) | ((((uint64)(e)) & 0x7ff) << 52) | ((m) & ((1ULL << 52) - 1))
#define FPD_UNPACK_VAR(f, s, e, m)                                                                                     \
    {                                                                                                                  \
        (s) = FPD_SIGN(f);                                                                                             \
        (e) = FPD_EXP(f) & 0x7ff;                                                                                      \
        (m) = FPD_FRAC(f);                                                                                             \
...
inline uint32 ppc_fpu_round(uint32 fpscr, ppc_double &d)
{
#if 1
    // .132
    switch (FPSCR_RN(fpscr)) {
    case FPSCR_RN_NEAR:
        if (d.m & 4) {
            // guard == 1
            if (d.m & 3) {
...
inline uint32 ppc_fpu_round_single(uint32 fpscr, ppc_single &s)
{
    switch (FPSCR_RN(fpscr)) {
    case FPSCR_RN_NEAR:
        if (s.m & 0x7) {
...
inline uint32 ppc_fpu_round_single(uint32 fpscr, ppc_double &s)
{
    switch (FPSCR_RN(fpscr)) {
    case FPSCR_RN_NEAR:
        if (s.m & 0x7) {
...
inline uint32 ppc_fpu_pack_double(uint32 fpscr, ppc_double &d, uint64 &res)
{
    // .124
    uint32 ret = 0;
    //	ht_printf("pd_type: %d\n", d.type);
...
inline uint32 ppc_fpu_pack_single(uint32 fpscr, ppc_double &d, uint32 &res)
{
    // .124
    uint32 ret = 0;
    switch (d.type) {
    case ppc_fpr_norm:
        ...
        //		ht_printf("ps: %qx: s:%d e:%d m:%qx\n", d, d.s, d.e, d.m);
        if (d.e > 0) {
            ret |= ppc_fpu_round_single(fpscr, d);
            if (d.m & (1ULL << 27)) {
                d.e++;
                d.m >>= 4;
            } else {
...
inline uint32 ppc_fpu_pack_double_as_single(uint32 fpscr, ppc_double &d, uint64 &res)
{
    ppc_single s;
    s.m = d.m >> 29;
    s.e = d.e;
    ...
    case ppc_fpr_norm:
        s.e = d.e + 127;
        if (s.e > 0) {
            ret |= ppc_fpu_round_single(fpscr, s);
            if (s.m & (1 << 27)) {
                s.e++;
                s.m >>= 4;
            } else {
    ...
    //	ht_printf("dm: %qx\n", d.m);
    ret |= ppc_fpu_pack_double(fpscr, d, res);
    return ret;
}

inline uint32 ppc_fpu_double_to_int(uint32 fpscr, ppc_double &d)
{
    switch (d.type) {
    case ppc_fpr_norm: {
        if (d.e < 0) {

...
void ppc_opc_faddx(PPC_CPU_State &aCPU)
{
	...
	{ uint64 t; aCPU.fpscr |= ppc_fpu_pack_double(aCPU.fpscr, D, t); aCPU.fpr[frD] = t; }
	if (aCPU.current_opc & PPC_OPC_Rc) {
		// update cr1 flags
		PPC_FPU_ERR("fadd.\n");
	}
...
void ppc_opc_faddsx(PPC_CPU_State &aCPU)
{
	...
	ppc_fpu_add(aCPU.fpscr, D, A, B);
	{ uint64 t; aCPU.fpscr |= ppc_fpu_pack_double_as_single(aCPU.fpscr, D, t); aCPU.fpr[frD] = t; }
	if (aCPU.current_opc & PPC_OPC_Rc) {
		// update cr1 flags
		PPC_FPU_ERR("fadds.\n");
	}
...
void ppc_opc_frspx(PPC_CPU_State &aCPU)
{
	...
	ppc_double B;
	ppc_fpu_unpack_double(B, aCPU.fpr[frB]);
	{ uint64 t; aCPU.fpscr |= ppc_fpu_pack_double_as_single(aCPU.fpscr, B, t); aCPU.fpr[frD] = t; }
	if (aCPU.current_opc & PPC_OPC_Rc) {
		// update cr1 flags
		PPC_FPU_ERR("frsp.\n");
	}

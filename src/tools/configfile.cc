...
static ObjectStream *createReadConfigFile(File *rfile, bool own_rfile, char magic[4], uint32 version)
{
	...
	// magic
	if (memcmp(magic, header.magic, 4) != 0) throw new MsgfException(
	...
	// version
	uint16 read_version;
	if (!hexw_ex(read_version, header.version)) throw new MsgfException(
	...
	// type
	uint8 read_type;
	if (!hexb_ex(read_type, header.type)) throw new MsgfException(
